...
func buildMultiKueueExternalFrameworkList(kueuelist []kueue.MultiKueueExternalFramework) []configapi.MultiKueueExternalFramework {
	ret := []configapi.MultiKueueExternalFramework{}
	for _, val := range kueuelist {
		ret = append(ret, configapi.MultiKueueExternalFramework{
			Name: fmt.Sprintf("%s.%s.%s", val.Kind, val.Version, val.Group),
		})
	}
	return ret
}
