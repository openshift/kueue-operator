...
type KueueLister interface {
	...
	List(selector labels.Selector) (ret []*kueueoperatorv1.Kueue, err error)
	// Get retrieves the Kueue from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*kueueoperatorv1.Kueue, error)
	KueueListerExpansion
}

// kueueLister implements the KueueLister interface.
type kueueLister struct {
	listers.ResourceIndexer[*kueueoperatorv1.Kueue]
}

// NewKueueLister returns a new KueueLister.
func NewKueueLister(indexer cache.Indexer) KueueLister {
	return &kueueLister{listers.New[*kueueoperatorv1.Kueue](indexer, kueueoperatorv1.Resource("kueue"))}
}
