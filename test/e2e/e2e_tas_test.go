...
var _ = Describe("TopologyAwareScheduling", Label("tas"), func() {
	...
	When("Creating a Job requesting TAS", func() {
		var (
		...
		BeforeEach(func(ctx context.Context) {
			...
			topology, cleanupTopo, rf, cleanupRF, _, cleanupCQ, localQueue, cleanupLQ = createTASResources(ctx, namespace.Name)
			_ = rf
		...
		It("should admit a Job via TAS", func(ctx context.Context) {
			...
			verifyWorkloadAdmittedWithTopology(ctx, namespace.Name, string(createdJob.UID), 1, topology.Name)
		})
	})

	When("Creating a JobSet requesting TAS", func() {
		var (
			namespace  *corev1.Namespace
			topology   *kueuev1beta2.Topology
			localQueue *kueuev1beta2.LocalQueue
		...
		It("should admit a JobSet via TAS", func(ctx context.Context) {
			...
			verifyWorkloadAdmittedWithTopology(ctx, namespace.Name, string(jobSet.UID), 2, topology.Name)
		})
	})

	When("Creating a Pod requesting TAS", func() {
		var (
			namespace  *corev1.Namespace
			localQueue *kueuev1beta2.LocalQueue
		)
		...
		BeforeEach(func(ctx context.Context) {
			...
			var cleanupTopo, cleanupRF, cleanupCQ, cleanupLQ func()
			_, cleanupTopo, _, cleanupRF, _, cleanupCQ, localQueue, cleanupLQ = createTASResources(ctx, namespace.Name)
			DeferCleanup(cleanupTopo)
			DeferCleanup(cleanupRF)
			DeferCleanup(cleanupCQ)
			DeferCleanup(cleanupLQ)
	...
	When("Creating a Deployment requesting TAS", func() {
		var (
			namespace  *corev1.Namespace
			localQueue *kueuev1beta2.LocalQueue
		)
